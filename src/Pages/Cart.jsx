                        ...
                        {cart.map((product)=>(
                            <div key={product.id} className='cart-item'>
                                ...
                                <div className='cart-item-quantity'>
                                    <button onClick={() => handleSubtractQuantity(product)}>-</button>
                                    <span>{product.quantity}</span>
                                    <button onClick={() => handleAddQuantity(product)}>+</button>
                                </div>
                                <p className='cart-item-price'>₹{product.price}.00</p>
                                <p className='cart-item-total'>₹{product.price * product.quantity}.00</p>
                                <button className='cart-item-remove' onClick={()=>handleRemoveItem(product)}><FaTrashAlt /></button>
                            </div>
                        ))}
                    </div>
